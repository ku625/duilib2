...
CStdStringPtrMap CDialogBuilder::m_createControlFunMap;
...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
			...
			if(pControl == NULL){
				LpCreateControlFuntion createFuntion = reinterpret_cast<LpCreateControlFuntion>(
					m_createControlFunMap.Find(pstrClass));
				if(createFuntion){
					pControl = createFuntion();
				}
			}
        }
...
void CDialogBuilder::AddCreateControlFuntion(LPCTSTR pstrClass, LpCreateControlFuntion fun)
{
	m_createControlFunMap.Insert(pstrClass, fun);
}
