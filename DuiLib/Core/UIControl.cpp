...
m_nTooltipWidth(300),
m_dragWindow(FOLLOW_PARENT),
m_bCustomToolTip(false)
{
    m_cXY.cx = m_cXY.cy = 0;
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if(_tcscmp(pstrName, _T("dragwindow")) == 0) {
		if(_tcscmp(pstrValue, _T("drag")))
		{
			SetDragWindow(DRAG_WINDOW);
		}
		else if(_tcscmp(pstrValue, _T("notdrag")))
		{
			SetDragWindow(NOT_DRAG_WINDOW);
		}
		else if(_tcscmp(pstrValue, _T("follow")))
		{
			SetDragWindow(FOLLOW_PARENT);
		}
	}
...
bool CControlUI::CanDragWindow()
{
	if(m_dragWindow == DRAG_WINDOW)
		return true;
	else if(m_dragWindow == NOT_DRAG_WINDOW)
		return false;
	else if(m_pParent)
		return m_pParent->CanDragWindow();
	else
		return true;
}

void CControlUI::SetDragWindow(DragWindow drag)
{
	m_dragWindow = drag;
}

bool CControlUI::IsCustomToolTip()
{
	return m_bCustomToolTip;
}

void CControlUI::SetCustomToolTip(bool custom)
{
	m_bCustomToolTip = custom;
}
