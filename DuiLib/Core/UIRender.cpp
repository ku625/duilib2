...
public:
	...
	~DCBuffer()
	{
		BYTE b = GetBValue(m_crCur);
		BYTE g = GetGValue(m_crCur);
		BYTE r = GetRValue(m_crCur);
		...
		LPALPHABLEND lpAlphaBlend = GetLPALPHABLEND();
		BOOL bRet = lpAlphaBlend(m_hdc, m_pRc->left, m_pRc->top, m_nWid, m_nHei, m_hMemDC, m_pRc->left, m_pRc->top, m_nWid, m_nHei, bf);
		::DeleteDC(m_hMemDC);
		::DeleteObject(m_hBmp);
		...
		bool bHole, bool bTiledX, bool bTiledY, bool bGray = false)
{
	...
	bool alphaChannel = pManager->IsBackgroundTransparent() ? true : data->alphaChannel;
	CRenderEngine::DrawImage(hDC, data->hBitmap, rcItem, rcPaint, rcBmpPart, rcCorner, alphaChannel, bFade, bHole, bTiledX, bTiledY);
                                          ...
                                          LPCTSTR pStrImage, LPCTSTR pStrModify)
{
    ...
    // mask='#FF0000' fade='255' hole='false' xtiled='false' ytiled='false' gray='fase'
...
{
	CRenderEngine::CheckAlphaColor(dwPenColor);
	ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
	LOGPEN lg;
...
{
	CRenderEngine::CheckAlphaColor(dwTextColor);
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
    if( pstrText == NULL || pManager == NULL ) return;
    ::SetBkMode(hDC, TRANSPARENT);
    ::SetTextColor(hDC, RGB(GetBValue(dwTextColor), GetGValue(dwTextColor), GetRValue(dwTextColor)));
    HFONT hOldFont = (HFONT)::SelectObject(hDC, pManager->GetFont(iFont));
	if(pManager->IsBackgroundTransparent())
	{
		DCBuffer dcBuf(hDC, &rc, 255, true);
		::DrawText(dcBuf, pstrText, -1, &rc, uStyle | DT_NOPREFIX);
		//AlphaDrawText(hDC, pManager, rc, pstrText, dwTextColor, iFont, uStyle);
	}
	else
	{
...
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, UINT uStyle)
{
	...
	CRenderEngine::CheckAlphaColor(dwBkColor);
            ...
            case _T('a'):  // Link
                {
					...
					CRenderEngine::CheckAlphaColor(clrColor);
						...
						if( pImageInfo ) {
                            ...
                            if( pt.x + iWidth > rc.right && pt.x > rc.left && (uStyle & DT_SINGLELINE) == 0 ) {
                                ...
                                if( bDraw && bLineDraw ) {
									...
									bool alphaChannel = pManager->IsBackgroundTransparent() ? true : pImageInfo->alphaChannel;
...
void CRenderEngine::AlphaDrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
{
	...
	graphics.SetTextRenderingHint(TextRenderingHint(TextRenderingHintSingleBitPerPixelGridFit));
