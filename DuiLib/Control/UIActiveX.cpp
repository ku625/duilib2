...
STDMETHODIMP CActiveXCtrl::GetWindowContext(IOleInPlaceFrame** ppFrame, IOleInPlaceUIWindow** ppDoc, LPRECT lprcPosRect, LPRECT lprcClipRect, LPOLEINPLACEFRAMEINFO lpFrameInfo)
{
	...
	//加上这个播放flash时内存一直增加，还没研究有什么用
	*ppFrame = NULL;//new CActiveXFrameWnd(m_pOwner);
	*ppFrame = NULL;
    *ppDoc = NULL;
    ACCEL ac = { 0 };
...
void CActiveXUI::ReleaseControl()
{
	...
	if(m_pControl != NULL)
	{
		if(m_pControl->m_pViewObject)
		{
			m_pControl->m_pViewObject->Release();
			m_pControl->m_pViewObject = NULL;
		}
		if(m_pControl->m_pInPlaceObject)
		{
			m_pControl->m_pInPlaceObject->Release();
			m_pControl->m_pInPlaceObject = NULL;
		}
