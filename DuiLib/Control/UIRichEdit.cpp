...
BOOL CTxtWinHost::TxSetScrollPos (INT fnBar, INT nPos, BOOL fRedraw)
{
	...
	SIZE newPos = m_re->GetScrollPos();
	SIZE sz = { newPos.cx - oldPos.cx, newPos.cy - oldPos.cy };
	m_re->OnTxSetScrollPos(sz);
    return TRUE;
}
...
BOOL CTxtWinHost::CanPaste(UINT nFormat /*= 0*/)
{
	if(NULL == pserv)
		return FALSE;
	HRESULT lRes = 0;
...
HRESULT CreateHostEx(CRichEditUI *re, const CREATESTRUCT *pcs, DuiLib::CTxtWinHost **pptec)
{
	HRESULT hr = E_FAIL;
	CTxtWinHost *phost = new CTxtWinHost();
	if(phost)
...
{
	return (m_lTwhStyle & ES_MULTILINE) > 0;
}
...
void CRichEditUI::OnTxSetScrollPos(SIZE changedSize)
{
